...
function App() {
  ...
  const calculateRestHours = () => {
    ...
    if (previousActivities.length === 0 || targetActivities.length === 0) {
      return "Need Prev Activ."; // No valid rest period if no activities exist
    }

    // Find the latest end time or pre/post from the previous date
    const lastEndTime = previousActivities.reduce((latest, activity) => {
      const end = new Date(`${previousDate}T${activity.end}`);
      const prePostEnd = new Date(
        end.getTime() + parseFloat(activity.prePost || "0") * 60 * 60 * 1000
      );
      return latest > prePostEnd ? latest : prePostEnd;
    }, new Date(0));

    // Find the earliest start time or pre/post from the target date
    const firstStartTime = targetActivities.reduce((earliest, activity) => {
      const start = new Date(`${targetDate}T${activity.start}`);
      const prePostStart = new Date(
        start.getTime() - parseFloat(activity.prePost || "0") * 60 * 60 * 1000
      );
      return earliest < prePostStart ? earliest : prePostStart;
    }, new Date("2099-01-01T00:00:00"));

    // Validate that lastEndTime and firstStartTime are valid dates
    if (isNaN(lastEndTime.getTime()) || isNaN(firstStartTime.getTime())) {
    ...
    return Math.max(0, restHours.toFixed(2)); // Prevent negatives & round to two decimal places
  };
  ...
  const getBoxStyle = (type, value) => {
    ...
    if (type === "flight") {
      ...
      // Add this block for rest hours
      if (value > 0 && value < 10) {
        backgroundColor = "#8B0000"; // Dark Red if greater than 0 and less than 10 hours
      } else if (value === 0) {
        backgroundColor = "#B8860B"; // Burnt Gold if 0 hours
      } else if (value >= 10) {
        backgroundColor = "grey"; // Burnt Gold if 10 or more hours
      }
    }
        ...
        >
          ...
          >
            ...
            <button
              ...
              onClick={() =>
                alert(
                  "2.10.3 Rest Period: A continuous period of 10 hours of uninterrupted time away from scheduled activities. This period should include 6 to 8 hours of sleep.\n\n***NOTE: Individuals have a personal responsibility to ensure they are rested with sufficient sleep when scheduled for duty activity.***"
                )
              }
            >
              Rest Period
